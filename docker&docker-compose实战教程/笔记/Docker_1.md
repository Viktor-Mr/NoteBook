...
1.本地镜像应该先打成tag，**名称要和远程仓库路径一致**

```shell
docker tag <image-name:tag> <username>/<repository>:<tag>
```

<image-name:tag>为本地构建的镜像名及版本号，<username>为Docker Hub账户名，<repository>为仓库名，<tag>为自定义的标签。

```sh
 docker tag mycentos:01  mrouou/mycentos:01
```

<img src="http://mk-images.tagao.top/img/image-20230718102308086.png?imageslim" alt="image-20230718102308086" style="zoom: 80%;" />
...
```shell
dpcker pull huangxiner/hzk3462:latest 
```
...
| 保留字         | 作用                                                         |
| -------------- | ------------------------------------------------------------ |
| **FROM**       | **当前镜像是基于哪个镜像的 （base 镜像）** `第一个指令必须是FROM` |
